...
export default function Berita() {
  const [umkmItems, setumkmItems] = useState<DocumentData[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const umkmDatas = await getDocs(collection(db, "umkm"));

        setumkmItems(
          umkmDatas.docs.map((doc) => ({ id: doc.id, ...doc.data() }))
        );
      } catch (err) {
  ...
  const totalPages = Math.ceil(umkmItems.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const currentItems = umkmItems.slice(startIndex, endIndex);
